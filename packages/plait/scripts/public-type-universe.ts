...
const reachesGeneratedCore = (
  ...
  const visitSymbol = (candidate: ts.Symbol): boolean => {
    ...
    for (const declaration of symbol.declarations ?? []) {
      ...
      if (generatedCoreAnchors.some((anchor) => anchor === relative)) return true
...
const renderInventory = (
  ...
  return [
    ...
    "<!-- Authority: emitted declarations from src/index.ts own the public type quantifier. Declaration-symbol ancestry reaching KernelCorpusSchemas or either generated kernel table derives from the generated core; every other type is ticketed debt with a named unification target. -->",
    "",
...
export const inspectPublicTypeUniverse = (
  ...
  const inspectExport = (
    ...
    if (target.flags & ts.SymbolFlags.Type) {
      ...
      const classification: ClassifiedPublicType = reachesGeneratedCore(checker, directory, target)
        ? {
            publicType: path,
            owner,
