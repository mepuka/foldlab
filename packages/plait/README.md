...
within a fixed backing-stream incarnation. Neither `Catalog` nor `Blobs` ships
a durable layer: the catalog layer is process-local, and the payload layer
answers absence until the object-store probe lands.
