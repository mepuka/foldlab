/**
 * Plane: kernel — the language's own machinery.
 *
 * The content-addressed daemon's operation shape and its inherited admission
 * route. The shape lives HERE, in the plane that consumes it, because a
 * service is a requirement and a requirement is named where it is required
 * (ruled at the 2026-08-19 sitting, F1): a built program's requirements
 * channel says `CasDaemon`, so the kernel declares what that means, and the
 * carriage plane's daemon — when its wiring slice lands — implements this
 * shape via its Layer. The former kernel→carriage type import inverts into
 * carriage→kernel, the lawful direction.
 *
...
import { admit as kernelAdmit } from "./KernelDoor.js"
