...
`test/TransportSpine.test.ts` is FH-1's stated deliverable. DEV-735 narrows
classification on this exact spine and now inherits a gated seam.

Amended again 2026-08-18 (operator ruling on the round-2 charge, same day): the
suite's table is DERIVED, never hand-listed. The first round-2 draft transcribed
the eight refusals into the test as literals; the ruling is that the table comes
from the same per-adapter data `internal/transport.ts` consumes. Each adapter
therefore exports `transportTerms: TransportTerms` and mints from it one line
below (`transportRefusalFor(transportTerms)`), and the wall iterates those
records — so an adapter cannot drift out of the table, and a source scan over
`src/internal` reds if any `transportRefusalFor` call site is missing from it or
mints from anything but its own exported terms. Alternatives, both refused: keep
the literal table (the ruling names it); derive and stop there.

Deriving and stopping there is the reason a second layer exists, and it is
stated plainly because it is the whole risk of the ruled mechanism: a table read
from the adapters agrees with the adapters BY CONSTRUCTION, so rewriting a law
moves both sides together and the derived layer stays green — the exact
homogenization this gate is charged with catching would pass. The derived layer
proves carriage (every field reaches the refusal intact, on the adapter's own
operation and a foreign one) and distinctness (eight laws, seven kinds, six
taught notes — the shape a homogenization collapses); it cannot prove fidelity
to the old text. So the emitted rows are frozen in
`fixtures/transport-refusals.ndjson` and regenerated byte-for-byte by the wall,
the discipline `fixtures/envelopes.ndjson` already runs under, with
`generate:transport-refusals` as the provenance line. The committed bytes'
authority is external to both sides: DEV-748 dumped all eight bindings at
`14298c2` — before this module existed — and at `963259d` and diffed them
byte-for-byte, and this seat re-checked all 64 strings in the emitted corpus
against the `14298c2` sources. **Load-bearing? yes** — the frozen layer is the
only part that can refute a homogenization; deleting it leaves a wall that
cannot fail.
...
Amended 2026-08-18 (DEV-748 round-2, minor charge; disposition RULED by the
operator the same day). As first drafted the new door was a pure function, so
it emitted no span, and every envelope decode — and every publish path that
re-enters the decode — lost the `Digest.digestOf` child span it used to emit.
This entry accepted that on the reasoning that the architecture rule governs
exported effectful functions and this door is neither. The ruling is that the
span is restored and named as before, and it is the better call: B-5's charter
is that it removes one redundant canonicalization and NOTHING else, and a trace
is observable, so a decode whose span tree changed shape would have made the
charter false in a way no assertion in this package would have caught.
`digestOfCanonicalBytes` is therefore `Effect.fn("Digest.digestOf")` — the span
name is deliberately the old function's, not this one's, because the point is
that a trace consumer sees exactly the child it saw before, in the same place.
The cost is one `yield*` at the call site and the span allocation the base
already paid. **Load-bearing? yes** as amended — the span name is the
observable the charter rests on.
...
Recorded for the record, since the charge asked for the exposure in writing and
not only its repair: `acquireConnection` resolves
`options.connectionName ?? defaultName`, so forwarding an options record makes
the connection name overridable by that record. The five adapters whose options
already carry `connectionName` — fabric client, registers, cells, lanes, folds —
had that override before the spine and keep it. Chaos never had it, and the
forwarding form would have granted it silently the day `RedeliveryChaosOptions`
grew the field; the call site passes `{ servers: options.servers }` so it
cannot.
