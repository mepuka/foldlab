...
describe("sound fold cache", () => {
  test("a cache hit byte-equals a fresh fold", () => {
    const head = headFrom(mergeSeed(), foldFixtureEvents)
    const fresh = primitiveFolds.sum.fold(foldFixtureEvents)
    ...
    const freshBytes = encodeFoldState(primitiveFolds.sum.fold(foldFixtureEvents))
