...
digests compared — is a passing test, not an intention. The current counts are
the output of `bun test`, not a durable claim to copy here; the wasm wall
explicitly skips when its gitignored `dist/` is absent. TypeScript and Go gates
are green. `proto/` runs its own gates, listed in `proto/AGENTS.md`; the model
gates run from `verify/*/run.sh`:
...
The KV combine implementation and executable law evidence
