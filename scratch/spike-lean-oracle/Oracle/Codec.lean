...
ASCII keys, non-negative integers, and ASCII-identifier strings. Inside that
grammar Lean's own printer already agrees with RFC 8785, so no separate
canonicaliser is needed on the Lean side.
-/
...
/-- The meaning fold only. `evidence` is ghost state and stays off the wire. -/
def stateJson (s : St) : Json :=
  Json.mkObj (holes.map fun h => (h.name, hstateJson (s.holes h)))
