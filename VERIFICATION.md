...
| Substrate model, CL-4 — lifecycle machine soundness over the transcribed status vocabulary | model-level R5 (Lean) | **Claimed** at the model level: total over the eleven transcribed symbols, readings held within states, transitions landing in the state their own event names, the terminal absorbing and reachable, drain and close two paths to it. The alphabet is DATA held against the transcription's own canonical rendering by an executed equality arm — name and placement, in row order — with its own drift control derived at gate time; a model-side reordering that every theorem survives reddens that arm alone. Timing carries no claim, and nothing here speaks about the vendor's implementation of the surface. The runtime read over the same table — per-session state as a fold over landed status facts — is a cited differential, not a refinement: it executes the three transition clauses over every position-and-symbol pair and refuses a symbol outside the alphabet, and it is not lockstep against this model's run. Bounds in the section below | [verify/substrate/](verify/substrate/), [go/daemon/wirevocabulary.canonical.json](go/daemon/wirevocabulary.canonical.json), [packages/plait/test/StatusPumpWall.test.ts](packages/plait/test/StatusPumpWall.test.ts), [packages/plait/test/ConnectionFold.test.ts](packages/plait/test/ConnectionFold.test.ts) |
...
| Substrate lifecycle command — succession, the fence through the shipped command, and health-gated readiness | runtime, executed walls with committed refutations | **Claimed** at the runtime level for one bounded schedule each. Succession: two starts and two teardowns over one store directory, every verb its own process running the shipped binary — the second start names the first as its predecessor, the chain walks total and acyclic in two steps, the drain and the stop leave distinguishable fact chains, and the shipped status report is read back for the words a fold cannot say. The fence is measured through the command: a round held out of band, and the start against it teaches a refusal carrying reason, law and repair while binding nothing (measured by dialling the port it was given) and landing nothing (measured by re-reading the lane). Readiness is executed both ways: the shipped probe admits a substrate whose JetStream answers one round trip with no retry, and REFUSES a real vendor server whose JetStream was taken down by the vendor's own verb, where the mutant reading — a bound listener — admits it. Bounds: a bounded schedule certifies its bounds and nothing beyond them; the fence is measured for concurrency and not for liveness, so a start run while an earlier incarnation is still serving is stopped by carriage rather than by the record; no bound is claimed on drain duration; and the health probe's own sentence is "JetStream is up if this server was configured for it", since the vendor returns healthy when the option was never set | [go/cmd/substrate/](go/cmd/substrate/), [go/cmd/lifecyclewall/](go/cmd/lifecyclewall/) |
| Plait shutdown posture — the scope-owned connection closes, undrained | runtime, executed wall + the chaos suite's committed mutant | **Claimed** at the runtime level for the seam it measures. Every position an awaited emission returned survives a close taken the instant the last one returned, with nothing beyond it on the stream; an un-awaited write on the same seam yields its caller no acknowledgement at all. A pump interrupted mid-tranche by closing its own scope leaves the consumer's acknowledgement floor at or below the anchor floor on every partition, and a resumed pump reaches an uninterrupted run's state digests. Bounds: the arms take the shutdown at the SCOPE and not at the process — the hard-kill evidence is the chaos suite's and is not inherited here; the anchor-before-ack premise's committed refutation is that suite's ack-before-anchor mutant; and nothing here claims what the substrate did with bytes a caller was never told about | [packages/plait/test/ShutdownPosture.test.ts](packages/plait/test/ShutdownPosture.test.ts), [packages/plait/test/FoldChaos.test.ts](packages/plait/test/FoldChaos.test.ts) |
...
the read-side fold over the facts those walls produce
([packages/plait/test/ConnectionFold.test.ts](packages/plait/test/ConnectionFold.test.ts)),
...
The fold suite executes the three transition clauses over every
position-and-symbol pair of the shipped table, refuses a planted symbol
outside the alphabet on every read it offers, and kills an executed
reading-promoted-to-a-state mutant on the placement column. It is a
differential over the RUNTIME table and its bound is stated where it is
run: it is not lockstep against this model's own run function over a
generated corpus, and no refinement map ties the two.
