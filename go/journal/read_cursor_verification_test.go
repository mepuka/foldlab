// Regression controls for issue #39: Read validates the cursor claim even
// when no later entry is available to expose a false head through Prev.
...
func TestReadAcceptsVerifiedCursorAfterAppend(t *testing.T) {
	js := startJetStream(t)
