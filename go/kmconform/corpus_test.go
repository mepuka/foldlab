...
func TestHeaderCountsMatchTheRecordsRead(t *testing.T) {
	...
	want := map[string]int{
		"kind": 12, "stage": 5, "refusal": 16, "type": 27, "run": 5,
		"encoding": 12, "admission": 19, "doc": 27, "canon": 10,
	}
	got := map[string]int{}
	for _, count := range corpus.Header.Counts {
...
func TestTables(t *testing.T) {
	...
	if len(corpus.Types) != 27 {
		t.Fatalf("%d type records, want the closed 27", len(corpus.Types))
	}
