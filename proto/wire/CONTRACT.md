...
`sessions.json` are generated by `proto/go/cmd/wirefix`; both sides re-derive
them independently (`proto/go/protod/wall_test.go`,
`proto/go/protod/session_test.go`, `proto/ts/test/wall.test.ts`,
`proto/ts/test/session-journal.test.ts`). Never edit; regeneration requires
...
- `sessions.json` — one `flb.session.v0` dialogue with each canonical event
  ...
  digest (U3 R0). `cmd/wirefix` drives the dialogue through a live daemon and
  records what it journaled, so a moved grammar digest is answered by rerunning
  the generator, never by editing the file; its `_provenance` names that
  command and both readers require the line.
