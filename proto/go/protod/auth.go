...
// every unauthenticated application connection a private subject namespace.
...
func newConnectionAuthenticator(internalPassword string) *connectionAuthenticator {
	return &connectionAuthenticator{internalPassword: internalPassword}
}
...
func (a *connectionAuthenticator) Check(client server.ClientAuthentication) bool {
	...
	if opts.Username != "" || opts.Password != "" || opts.Token != "" || opts.Nkey != "" || opts.JWT != "" {
		return false
	}
