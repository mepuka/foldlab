...
## Task 21 — float leaf drop (2026-08-16; D109-D111 assigned at merge f2d3b9c0d)

### D109. The removed float leaf uses the existing unknown-kind refusal
...
### D110. Every active grammar mirror hard-cuts under the existing v0 name
...
### D111. Regenerate only fixtures whose bytes depend on the removed leaf
...
## Task 25 — float/hygiene cure (2026-08-17; D112-D121 assigned at merge f2d3b9c0d)
...
### D112. `sessions.json` is generated by driving a live daemon, not recomputed
...
### D113. The generator learns the grammar digest from the daemon's refusal
...
### D114. One integrality bound, stated once, governs every v0 number position
...
### D115. The session grammar descriptor restates the narrowed literal production
...
### D116. Regenerating `sessions.json` is the whole fixture consequence
...
### D117. The session grammar digest hard-cuts; pre-cut journals do not open
...
### D118. `opaque` keeps its whole admission, and the law is written down
...
### D119. The restatement survey lands in `proto/GRAMMAR-SITES.md`
...
### D120. The grep guard covers the fourteen sites inside `proto/`
...
### D121. The spec's do-not-edit law gains a recorded exception
...
## Task 26 — the closure law (2026-08-17; D122-D132 assigned at merge f2d3b9c0d)

### D122. The integrality bound moves to the number-decoding path, not to the next position
...
### D123. The closure is guarded by a one-call-site rule, not by a vector list
...
### D124. The refusal coordinate is identity-ordered, not map-ordered
...
### D125. The TypeScript mirror is one traversal over the folded term
...
### D126. The session grammar descriptor commits the law once, term-wide
...
### D127. Regenerating `sessions.json` is again the whole fixture consequence
...
### D128. The grammar block and the certifier are checked for agreement
...
### D129. The regeneration claim gets a gate, five fixtures at a time
...
### D130. The regeneration gate runs with `-count=1`, in a stage of its own
...
### D131. `verify/ir`'s abstraction notes are corrected in prose only
...
### D132. New citations name the brief by number; existing paths stay as written
...
## Task 27 — the final hygiene round (2026-08-17; D133-D137 assigned at merge f2d3b9c0d)

### D133. Cache-defeat is armed at the STAGE, on every Go test stage
...
### D134. `requireIntegralNumbers` refuses its whole non-JSON domain, not a list of numeric types
...
### D135. The TS closure sweeps before the mint, and the traversal moves to `jcs.ts`
...
### D136. `verify/ir`'s abstraction notes state the second conjunct and where it lives
...
### D137. A pre-law catalog fact stays resolvable by digest, and no migration is built
