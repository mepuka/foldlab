...
import { applyKV, emptyKV, headFrom, streamSeed, type StreamEvent } from "@foldlab/core/stream"
// The ONE import to change when `@effect-atom/atom` is installed. See atom.ts.
import { Atom, Registry } from "./atom.ts"

const banner = (text: string): void => console.log(`\n${"─".repeat(66)}\n${text}\n`)
...
console.log(`  constructors : Deposited | Withdrew | Labelled`)
console.log(`  history      : ${history.map((e) => e._tag).join(" → ")}`)
...
const ledger = defineFold(
  ...
  productStep(steps.constOne, steps.payloadNumber(["amount"])),
)

banner("B. THE ONE FOLD — the reducer (house name: the meaning fold)")
const wired = onWire(history)
const state = ledger.fold(wired)
console.log(`  events folded : ${history.length}`)
console.log(`  fold state    : [count=${state[0]}, max=${state[1]}]`)
console.log(`  fold digest   : ${short(ledger.digest!)}`)
...
console.log(`  chain head        : ${short(head)}`)
console.log(`  fold state canon. : ${stateBytes.ok ? stateBytes.bytes : "REFUSED"}`)
...
if (read.ok && read.hit) {
  console.log(`  cached bytes  : ${read.bytes}`)
  check("the cached value is what a fresh fold would produce", read.bytes === stateBytes.bytes)
}
...
console.log(`  sequential : ${stateBytes.ok ? stateBytes.bytes : "REFUSED"}`)
console.log(`  left ⊕ right: ${combinedBytes.ok ? combinedBytes.bytes : "REFUSED"}`)
check(
...
const refusal = Effect.runSync(Effect.flip(applyKV(emptyKV, poison)))
console.log(`  refusal tag   : ${refusal._tag}`)
console.log(`  refused seq   : ${refusal.event.seq}`)
check("a NUL key is refused as a typed value, not thrown", refusal._tag === "MalformedPayload")
check("and no state was produced to be corrupted", emptyKV.count === 0)
...
registry.subscribe(stateAtom, (next) => {
  seen.push("state")
  console.log(`  → state  : [count=${next[0]}, max=${next[1]}]`)
})
registry.subscribe(digestAtom, (next) => {
  seen.push("digest")
  console.log(`  → digest : ${short(next)}`)
})

console.log(`  initial state  : [count=${registry.get(stateAtom)[0]}, max=${registry.get(stateAtom)[1]}]`)
console.log(`  initial digest : ${short(registry.get(digestAtom))}`)
console.log("  appending Deposited(900) …")
