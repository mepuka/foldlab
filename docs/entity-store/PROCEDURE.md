...
|---|---|---|---|---|
| `KICKOFF.md` | program record: theses, case-study synthesis, ruling waves (R-, G-series) | orientation; ruling authority lookup | append numbered sections per thrust or wave; existing text changed only by dated addendum | numbered §; ruling tables carry an authority column |
| `STORE-MODEL.md` | the model specification: identity scheme, invariants, operations and laws, kinds, theorem inventory (M-series), joint rulings (Q-series), amendment records (A-series) | the model's single authority; statement pinning reads it | amendment only (Q10 discipline): dated paragraphs in §7/§9; falsifications recorded, never erased | §1–§9 fixed; inventory table one row per M-statement |
| `STORE-SHELL.md` | the executable-store specification: claim ladder, IO whitelist, layout, boundary, harness, SH-rulings, delivery records | the shell's single authority | delivery records append to §9; rung or whitelist changes only by ruling | §1–§9 fixed; §8 is the ruling record |
...
| `LEDGER.md` (§8, planned) | the machine-extracted proved ledger, both sides | the mechanical statement of what is proved and gated | REGENERATED only; hand edits are violations the diff catches | generated; banner line 1 |
