...
### Oracle
- **Kind:** adr (role designation). **Ruling:** CV-1.
- **Form:** the Lean estate's role toward the production implementation: the reference
  whose proven theorems and generated artifacts gate what the monorepo may ship. The
  oracle gates; the monorepo runs.
- **Obligations:** every gate the oracle exports derives from a proved theorem, a
  committed script, or generated bytes — never hand-written assertion.
- **Avoid:** never "the oracle decides production behavior" (it gates cutover, it does
  not execute production); never call the monorepo a source of semantic truth; never
  call anything an oracle verdict that lacks a LEDGER-traceable basis.

### Conformance bundle
- **Kind:** schema (versioned data with laws). **Ruling:** CV-1.
- **Form:** the estate's exported, versioned artifact the monorepo pins: golden vectors,
  the wire-format specification, the script corpus, the theorem-contract table (from
  LEDGER.md).
- **Obligations:** byte-deterministic; contents generated or proven only; a change is a
  new version, never a mutation; consumed by pin (SHA + digest), never floating.
...
- **Form:** one unit of the cutover (codecs, digest, canon/admission, store engine,
  names, graph/verification, typing/API, transport), each with exactly one gate: its
  bundle slice green in the monorepo's CI plus the theorem contracts it reifies.
- **Obligations:** a green station never regresses (the ratchet); the conformance suite
  names the first red station — each run tells the next step.
- **Avoid:** a station with two gates or none; cutting over a station whose backing
  theorems are stated-unproved without the gate output saying so; reordering stations
  past their dependency edges.
