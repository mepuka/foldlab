...
| evidence lane | one stream per declared `(lane, partition)`, limits-based retention by declared policy (ruled 2026-08-17, DEV712-POS-1: the partition stream's dense sequence IS F2b's position coordinate by construction — the transport coordinate and the model coordinate coincide, which is this plane's center-of-gravity sentence made literal; the spine's single shared stream was a slice-0 bootstrap and is re-scoped to the request/control plane) | at-least-once delivery; durable consumers; per-partition order = per-stream order | exactly-once (dedup window is bounded); cross-partition order (none exists); any position coordinate other than the partition stream's own sequence |
