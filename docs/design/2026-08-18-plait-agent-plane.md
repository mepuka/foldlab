...
| The wire contract is bytes: canonical envelopes, closed struct, excess refused; refusal envelope carries kind/sort/law/path/next; the envelope has no timestamp field | shipped (`packages/plait/src/kernel/Wire.ts`) | the shuttle's entire node contract (§8); the timestamp audit (§11.4) |
