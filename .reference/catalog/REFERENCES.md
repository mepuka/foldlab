...
| [AXLE: A Cloud Infrastructure for Lean 4 Theorem Proving Utilities](https://arxiv.org/abs/2606.26442) | Tooling paper; local v1 pinned by the [receipt](../provenance/receipts/axle-paper.json) | Cloud proof-manipulation and strict-verification service architecture; its service claims establish no proof result or independent trust for this estate |
...
| [Aeneas use case](https://lean-lang.org/use-cases/aeneas/), [repository](https://github.com/AeneasVerif/aeneas), and [local paper receipt](../provenance/receipts/aeneas-paper.json) | Pattern/adapt | Functional translation, explicit supported Rust subset, extrinsic proofs, generated versus handwritten models | Safe-Rust subset; no transferred guarantee for TypeScript |
...
| [Selective Applicative Functors](https://doi.org/10.1145/3341694) (Mokhov, Lukyanov, Marlow, Dimino, ICFP 2019, PACMPL 3(ICFP) art. 90) — **corpus pin pending**, not yet in the [paper lock](../provenance/papers.lock.json) | Vocabulary | The rung between applicative and monadic: `select` keeps static effect visibility while adding conditional execution; `Over`/`Under` as the over- and under-approximating carriers, and rigidity as their side condition; the rigid normal form as a left-associated linear sequence, i.e. a table; `select = selectM` forced on any monad, which is why a fragment must be a separate carrier and never a `Selective` instance on `Prog`. Consumed by `Cas/Lang/Fragments.lean` | Vocabulary and positioning only; no claim in `library/cas` depends on SAF being correct. SAF asserts `necessary ⊆ actual ⊆ possible` and does not prove it — the estate's proved instance for the table rung is `Cas/Lang/Defun.lean`, stated against the word, and is not a transferred guarantee |
