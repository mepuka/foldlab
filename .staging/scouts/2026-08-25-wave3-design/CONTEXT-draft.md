# CONTEXT — the Entity Store context (DRAFT for grilling)

Status: staged draft, 2026-08-25 — no claim gate satisfied. Ratifying this document is
migration step 2 (ruling W3-6): it discharges **R-1** (context home), takes the
PROCEDURE §7 registry row already ruled (W3-1), and adds the context's row to
`CONTEXT-MAP.md`. Until ratified, nothing here is minted. Kind: **glossary**.

## R-1, the ruling this document carries

**Proposed: the context is named "Entity Store", home `docs/entity-store/`** (already
the de-facto home; no files move). **The Lean namespace stays `E2`**, recorded below as
the context's code label — this discharges R-1 without the maximal-blast-radius rename
(every module, theorem, and seat import), which buys nothing semantic. Alternative if
the operator wants the rename: it is a serialization point, scheduled tree-quiet, as
R-F priced it. STORE-MODEL §7's "working labels until R-1" clause resolves either way.
...
1. R-1: the name, and keep-`E2` vs rename.
2. Each term's avoid-list — are the fences right?
3. `address-commits-to-encoding`: promote as minted rule, or leave as anatomy prose?
4. Anything minted here the domain modeling review wants worded differently.
