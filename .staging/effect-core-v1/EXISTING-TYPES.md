...
| `DirectHandler` / `HandlerEnv` | `Handler`; `Handler.sum`; `Handler.through` | Finite typed lookup, region scoping, ownership, and explicit delegation around existing semantic handlers. | elaboration to `Handler`; lookup uniqueness; restoration/frame laws. |
