...
| `EC1-CE045` | Sequencing a finalizer with ordinary `Prog.bind` runs it when the body refuses. | `ensuring_never_finalises_a_refusal`, `ensuring_witness`. | `VERIFIED-KERNEL`; `interpretRef_bind` is refusal-strict. `ensuring` needs a target or machine clause that observes both success and failure. |
| `EC1-CE046` | EffHOL's (Mod-E) transfers unconditionally to `PProg` with `++` and existing `wlp`. | The empty prefix satisfies every `wlp` postcondition by refusal while its appended suffix need not; `wlp_falsifier_empty_prefix`, `modE_unsound_at_the_empty_prefix`. | `VERIFIED-KERNEL`; use the existing nonempty-prefix, history-threaded composition theorem rather than importing the paper rule verbatim. |
| `EC1-CE047` | A nonempty-prefix premise alone makes the table-only reading of (Mod-E) valid. | A suffix with answer references refuses when restarted alone but succeeds under the prefix history; `modE_unsound_at_a_restarted_history`. | `VERIFIED-KERNEL`; the compositional modality is indexed by table plus answer history, exactly as `wlp_append` states. |
...
| --- | --- | --- | --- |
| `EC1-CE042` | Full Effect Core has a globally unique denotation without fixing choices. | Required witness: one admitted program with two typed decision/schedule streams and two permitted observations. `OWED`. | The public semantics remains relational; only fixed-decision and deterministic-subfragment uniqueness may be stated. |
