...
| **The substrate gate** (`go/substrate/`, `packages/plait/test/SubstrateParity.test.ts`) | The JetStream properties the design consumes, probed executably rather than assumed: subject CAS, KV revision lifecycle, CAS-before-dedup precedence, the application ACL scope, SIGKILL process recovery, and TS-client parity with the refusals pinned wire-indistinguishable. | [Substrate assumptions gate](VERIFICATION.md#status-at-a-glance) — executable integration at the pinned envelope; single node, R=1, file storage; process-crash recovery only, never power loss; no clustering |
