...
`scripts/check-laws.ts`) is the prior art this tool re-adopts in Lean.
...
red on that would be deleted within a week.
...
def document (reg : List Ruling) (cs : List Claim) : String :=
...
private def baseDoc : String := document synthReg synthClaims
...
unsafe def gate (args : List String) : IO Unit := do
  ...
  let fixtures' : IO (List Gate.Fixture) := do
    ...
    return [⟨outPath, Law.document Law.registry cs,
