...
verification oracle at its simplest honest instantiation. The vector
families use the canonical-bytes toy-digest oracle instead; the tie to
the full CAS admission judgment is R2's refinement obligation. -/
